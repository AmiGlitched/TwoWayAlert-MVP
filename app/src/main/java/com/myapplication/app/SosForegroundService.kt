...
private var isFallDetectionEnabled=false
    // COOLDOWN: Prevents retriggering for 60 seconds after a fall is detected
    private var isFallCooldownActive = false
    private var fallCooldownEndTime = 0L
    private val FALL_COOLDOWN_MS = 60000L // 60 seconds
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        isFallDetectionEnabled = intent?.getBooleanExtra("ENABLE_FALL_DETECTION", false) ?: false
    ...
    private fun simulateFallEvent() {
        ...
        Log.d("MLTest", "Simulated Prediction: $result")
    ...
    override fun onSensorChanged(event: SensorEvent?) {
        if (event?.sensor?.type == Sensor.TYPE_ACCELEROMETER) {
            ...
            // If the phone is perfectly still, magnitude ≈ GRAVITY_EARTH (9.8 m/s²)
            // Allow a small tolerance (±1.0 m/s²) for sensor noise.
            val isStill = kotlin.math.abs(magnitude - SensorManager.GRAVITY_EARTH) < 1.0f


            // Shake-to-SOS gesture — only runs when explicitly toggled on.
            if (shakeTriggerEnabled) {
                // Pass the raw magnitude (m/s²) for shake detection
                checkForShake(magnitude)
            }

            // ML fall detection — only predicts when explicitly enabled.
            if (isFallDetectionEnabled) {
                // If the phone is completely still, skip ML entirely.
                // A real fall ALWAYS involves movement (free-fall + impact).
                if (isStill) {
                    // Clear any stale samples to prevent delayed false triggers
                    if (buffer.isNotEmpty()) {
                        buffer.clear()
                    }
                    return // Exit early, don't process or buffer
                }

                // Convert sensor values from m/s² to g (matching training data)
                ...
                if (buffer.size >= windowSize) {
                    ...
                    // Prevents repeated triggering for 60 seconds after a fall is detected.
                    if (isFallCooldownActive) {
                        if (System.currentTimeMillis() < fallCooldownEndTime) {
                            return // Skip the model entirely during cooldown
                        } else {
                            isFallCooldownActive = false // Cooldown expired
                        }
                    }
                    ...
                    if (result == "Fall") {
                        ...
                        // Ignore all ML predictions for the next 60 seconds
                        isFallCooldownActive = true
                        fallCooldownEndTime = System.currentTimeMillis() + FALL_COOLDOWN_MS
                ...
                // Clear stale samples if fall detection is toggled off
                buffer.clear()
            }
        }
    }
